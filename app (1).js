...
function renderFree() {
  ...
  // No window may be shorter than MIN_FREE. When two time points are too close,
  // drop the less round one (1:55 goes before 2:00). The day's start and end always stay.
  const roundness = (m) => (m === dayStart || m === dayEnd ? 9 : m % 30 === 0 ? 2 : m % 10 === 0 ? 1 : 0);
  for (let i = 0; i < pts.length - 1; ) {
    if (pts[i + 1] - pts[i] >= MIN_FREE) { i++; continue; }
    const dropNext = roundness(pts[i + 1]) <= roundness(pts[i]);
    pts.splice(dropNext ? i + 1 : i, 1);
    if (!dropNext && i > 0) i--; // recheck the window before
  }
  const windows = [];
  ...
  // Fold any window shorter than MIN_FREE into its neighbour. The combined
  // window only lists rooms that are free for the whole of it.
  let short;
  while (windows.length > 1 && (short = windows.findIndex((w) => w.b - w.a < MIN_FREE)) !== -1) {
    const i = short === 0 ? 0 : short - 1; // merge windows i and i+1
    const [x, y] = [windows[i], windows[i + 1]];
    const merged = { a: x.a, b: y.b, free: x.free.filter((r) => y.free.includes(r)) };
    windows.splice(i, 2, merged);
    // re-join with neighbours that now have the same rooms
    for (let j = windows.length - 1; j > 0; j--) {
      if (windows[j].free.join() === windows[j - 1].free.join()) {
        windows[j - 1].b = windows[j].b;
        windows.splice(j, 1);
      }
    }
  }
